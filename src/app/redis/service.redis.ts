...
export class RedisAllCustomServiceOop {
  ...
  async findDataByUserIdAndSocketId(userId: string, socketId: string) {
    const getUsers = await this.globalRedis.get(
  ...
  async findAllDataByKeyScan(key: string) {
    ...
    do {
      const [newCursor, keys] = await this.globalRedis.scan(
        cursor,
        'MATCH',
        key + '*',
        'COUNT',
    ...
    if (keys.length) {
      getData = await this.globalRedis.mget(keys);
    }
    return getData;
  }
  ...
  async findAllKeysScan(key: string) {
    let cursor = '0';
